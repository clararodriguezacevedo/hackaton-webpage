...
export default function Mentor({ mentor, className }) {
  const revealed = mentor.revealed !== false
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <p className="text-sm text-center mt-2">{mentor.name}</p>
      </div>
      ...
      <DialogContent className="z-[999] bg-[#14192D] border-2 border-gray-700 rounded-md max-w-2xl max-h-[90vh] overflow-y-auto [&>button.absolute]:hidden">
        <DialogHeader className="flex flex-row items-center justify-between">
          <DialogTitle className="text-2xl font-semibold">{mentor.name}</DialogTitle>
          <DialogClose asChild>
        ...
        <div className="flex flex-col space-y-12 pt-8 text-center">
          ...
          {mentor.linkedin && (
            <div className="mt-4">
              <a href={mentor.linkedin} target="_blank" rel="noopener noreferrer" className="inline-block text-white hover:text-white/80">
